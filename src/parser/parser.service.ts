...
export class ParserService {
  ...
  private async getGroups() {
    ...
    $('.vt252').each((i, faculty) => {
      ...
      $(faculty).find($('.vt256')).each((i, group) => {
  ...
  private async getSchedule() {
    ...
    }, {weekStartsOn: 2}).map((d) => d.toISOString().split('T')[0]);

    for (let i in weeks) {
      const currentDate = weeks[i];
      const $ = await this.getHTML(`${URL}?group=${groupId}&date=${currentDate}`);

      $('.vt237').slice(1).each((currCol, column) => {
        const rawDate = $(column).text().replace($(column).find('.vt238').text(), '').trim();
        const dateDDMM = rawDate.split('.');
        const date = `${new Date().getFullYear()}-${dateDDMM[1]}-${dateDDMM[0]}`
        const dayOfWeek = $(column).find('.vt238').text().trim();

        $('.vt244b').find('.vt244').each((currRow, row) => {
          const pairNumAndTime = $(row).find('.vt239').eq(0);
          const pairNum = pairNumAndTime.find('.vt283').text();
          const pairTime = pairNumAndTime.text().replace(pairNum, '').match(/[0-9]{2}:[0-9]{2}/g);
          const pairStart = pairTime[0];
          const pairEnd = pairTime[1];
          const pairsOnTime = $(row).find('.vt239').slice(1).eq(currCol).find('.vt258');
